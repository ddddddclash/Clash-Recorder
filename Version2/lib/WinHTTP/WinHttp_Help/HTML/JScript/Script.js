...
searchDB.push(new searchOption("_WinHttpAddRequestHeaders", "Adds one or more HTTP request headers to the HTTP request handle."));
searchDB.push(new searchOption("_WinHttpCheckPlatform", "Determines whether the current platform is supported by this version of Microsoft Windows HTTP Services (WinHTTP)."));
searchDB.push(new searchOption("_WinHttpCloseHandle", "Closes a single handle."));
searchDB.push(new searchOption("_WinHttpConnect", "Specifies the initial target server of an HTTP request and returns connection handle to an HTTP session for that initial target."));
searchDB.push(new searchOption("_WinHttpCrackUrl", "Separates a URL into its component parts such as host name and path."));
searchDB.push(new searchOption("_WinHttpCreateUrl", "Creates a URL from array of components such as the host name and path."));
searchDB.push(new searchOption("_WinHttpDetectAutoProxyConfigUrl", "Finds the URL for the Proxy Auto-Configuration (PAC) file."));
searchDB.push(new searchOption("_WinHttpGetDefaultProxyConfiguration", "Retrieves the default WinHttp proxy configuration."));
searchDB.push(new searchOption("_WinHttpGetIEProxyConfigForCurrentUser", "Retrieves the Internet Explorer proxy configuration for the current user."));
searchDB.push(new searchOption("_WinHttpOpen", "Initializes the use of WinHttp functions and returns a WinHttp-session handle."));
searchDB.push(new searchOption("_WinHttpOpenRequest", "Creates an HTTP request handle."));
searchDB.push(new searchOption("_WinHttpQueryAuthSchemes", "Returns the authorization schemes that are supported by the server."));
searchDB.push(new searchOption("_WinHttpQueryDataAvailable", "Returns the availability to be read with _WinHttpReadData()."));
searchDB.push(new searchOption("_WinHttpQueryHeaders", "Retrieves header information associated with an HTTP request."));
searchDB.push(new searchOption("_WinHttpQueryOption", "Queries an Internet option on the specified handle."));
searchDB.push(new searchOption("_WinHttpReadData", "Reads data from a handle opened by the _WinHttpOpenRequest() function."));
searchDB.push(new searchOption("_WinHttpReceiveResponse", "Waits to receive the response to an HTTP request initiated by WinHttpSendRequest()."));
searchDB.push(new searchOption("_WinHttpSendRequest", "Sends the specified request to the HTTP server."));
searchDB.push(new searchOption("_WinHttpSetCredentials", "Passes the required authorization credentials to the server."));
searchDB.push(new searchOption("_WinHttpSetDefaultProxyConfiguration", "Sets the default WinHttp proxy configuration."));
searchDB.push(new searchOption("_WinHttpSetOption", "Sets an Internet option."));
searchDB.push(new searchOption("_WinHttpSetStatusCallback", "Sets up a callback function that WinHttp can call as progress is made during an operation."));
searchDB.push(new searchOption("_WinHttpSetTimeouts", "Sets time-outs involved with HTTP transactions."));
searchDB.push(new searchOption("_WinHttpSimpleBinaryConcat", "Concatenates two binary data returned by _WinHttpReadData() in binary mode."));
searchDB.push(new searchOption("_WinHttpSimpleFormFill", "Fills web form."));
searchDB.push(new searchOption("_WinHttpSimpleReadData", "Reads data from a request"));
searchDB.push(new searchOption("_WinHttpSimpleReadDataAsync", "Reads data from a request in asynchronous mode"));
searchDB.push(new searchOption("_WinHttpSimpleRequest", "A function to send a request in a simpler form"));
searchDB.push(new searchOption("_WinHttpSimpleSendRequest", "A function to send a request in a simpler form, but not read the data"));
searchDB.push(new searchOption("_WinHttpSimpleSendSSLRequest", "A function to send a SSL request in a simpler form, but not read the data"));
searchDB.push(new searchOption("_WinHttpSimpleSSLRequest", "A function to send a SSL request in a simpler form"));
searchDB.push(new searchOption("_WinHttpTimeFromSystemTime", "Formats a system date and time according to the HTTP version 1.0 specification."));
searchDB.push(new searchOption("_WinHttpTimeToSystemTime", "Takes an HTTP time/date string and converts it to array (SYSTEMTIME structure values)."));
searchDB.push(new searchOption("_WinHttpWriteData", "Writes request data to an HTTP server."));
