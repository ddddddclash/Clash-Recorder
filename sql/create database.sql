...
CREATE TABLE IF NOT EXISTS `clashtrack`.`clan` (
  ...
  `date_made` DATETIME NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `clashtrack`.`profile` (
  ...
  `date_made` DATETIME NULL COMMENT '',
...
-- -----------------------------------------------------
-- Table `clashtrack`.`player`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `clashtrack`.`player` (
...
CREATE INDEX `fk_player_profile1_idx` ON `clashtrack`.`player` (`profile_profile_id` ASC)  COMMENT '';

CREATE INDEX `fk_player_clan1_idx` ON `clashtrack`.`player` (`clan_clan_id` ASC)  COMMENT '';

CREATE INDEX `fk_player_war1_idx` ON `clashtrack`.`player` (`war_war_id` ASC)  COMMENT '';
...
CREATE TABLE IF NOT EXISTS `clashtrack`.`war_event` (
  ...
  `time_till_war_end` VARCHAR(45) NULL COMMENT '',
  ...
  CONSTRAINT `fk_war_event_player1`
    ...
    REFERENCES `clashtrack`.`player` (`player_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `clashtrack`.`clan_snapshot` (
  `clan_snapshot_id` INT NOT NULL COMMENT '',
  `clan_id` INT NOT NULL COMMENT '',
  `date_made` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '',
  `screenshot` MEDIUMBLOB NULL COMMENT '',
  `badge` BLOB NULL COMMENT '',
  ...
  `clan_experience` INT NULL COMMENT '',
  `clan_experience_goal` INT NULL COMMENT '',
  PRIMARY KEY (`clan_snapshot_id`, `clan_id`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `clashtrack`.`profile_snapshot` (
  ...
  `date_made` DATETIME NULL COMMENT '',
  `screenshot` MEDIUMBLOB NULL COMMENT '',
  `player_name` VARCHAR(45) NULL COMMENT '',
  `player_poition` INT NULL COMMENT '',
  `trophees` INT NULL COMMENT '',
